...
class CwlAnalysisInput(BaseModel):
    ...
    actual_instance: Optional[
        Union[CwlAnalysisJsonInput, CwlAnalysisStructuredInput]
    ] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, CwlAnalysisStructuredInput):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `CwlAnalysisStructuredInput`"
            )
        else:
            match += 1
        ...
        if not isinstance(v, CwlAnalysisJsonInput):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `CwlAnalysisJsonInput`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in CwlAnalysisInput with oneOf schemas: CwlAnalysisJsonInput, CwlAnalysisStructuredInput. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in CwlAnalysisInput with oneOf schemas: CwlAnalysisJsonInput, CwlAnalysisStructuredInput. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into CwlAnalysisInput with oneOf schemas: CwlAnalysisJsonInput, CwlAnalysisStructuredInput. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into CwlAnalysisInput with oneOf schemas: CwlAnalysisJsonInput, CwlAnalysisStructuredInput. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        ...
        Union[Dict[str, Any], CwlAnalysisJsonInput, CwlAnalysisStructuredInput]
    ]:
