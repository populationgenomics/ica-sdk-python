...
class UploadRule(BaseModel):
    ...
    local_folder: Annotated[str, Field(min_length=1, strict=True, max_length=255)] = (
        Field(
            description="The local folder to monitor. Files in this folder on your local environment will be uploaded to the specified project. Only files matching the filePattern will be uploaded.",
            alias="localFolder",
        )
    )
    file_pattern: Annotated[str, Field(min_length=1, strict=True, max_length=255)] = (
        Field(
            description="The regular expression to match a file name. eg: to match all files use '.*'",
            alias="filePattern",
        )
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "dataFormat": DataFormat.from_dict(obj["dataFormat"])
                if obj.get("dataFormat") is not None
                else None,
