...
class TempCredentials(BaseModel):
    ...
    aws_temp_credentials: Optional[AwsTempCredentials] = Field(
        default=None, alias="awsTempCredentials"
    )
    rclone_temp_credentials: Optional[RcloneTempCredentials] = Field(
        default=None, alias="rcloneTempCredentials"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if (
            self.aws_temp_credentials is None
            and "aws_temp_credentials" in self.model_fields_set
        ):
        ...
        # and model_fields_set contains the field
        if (
            self.rclone_temp_credentials is None
            and "rclone_temp_credentials" in self.model_fields_set
        ):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "awsTempCredentials": AwsTempCredentials.from_dict(
                    obj["awsTempCredentials"]
                )
                if obj.get("awsTempCredentials") is not None
                else None,
                "rcloneTempCredentials": RcloneTempCredentials.from_dict(
                    obj["rcloneTempCredentials"]
                )
                if obj.get("rcloneTempCredentials") is not None
                else None,
            }
        )
        return _obj
