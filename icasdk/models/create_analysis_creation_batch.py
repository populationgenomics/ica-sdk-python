...
class CreateAnalysisCreationBatch(BaseModel):
    ...
    nextflow_items: Optional[List[CreateNextflowAnalysis]] = Field(
        default=None, alias="nextflowItems"
    )
    nextflow_json_items: Optional[List[CreateNextflowJsonAnalysis]] = Field(
        default=None, alias="nextflowJsonItems"
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "cwlItems": [
                    CreateCwlAnalysis.from_dict(_item) for _item in obj["cwlItems"]
                ]
                if obj.get("cwlItems") is not None
                else None,
                "nextflowItems": [
                    CreateNextflowAnalysis.from_dict(_item)
                    for _item in obj["nextflowItems"]
                ]
                if obj.get("nextflowItems") is not None
                else None,
                "nextflowJsonItems": [
                    CreateNextflowJsonAnalysis.from_dict(_item)
                    for _item in obj["nextflowJsonItems"]
                ]
                if obj.get("nextflowJsonItems") is not None
                else None,
            }
        )
        return _obj
