...
class ActivationCodeDetail(BaseModel):
    ...
    id: UUID
    allowed_slots: Annotated[int, Field(strict=True, ge=-1)] = Field(
        description="The allowed slot within this code, -1 means unlimited",
        alias="allowedSlots",
    )
    used_slots: Annotated[int, Field(strict=True, ge=0)] = Field(
        description="Indicates how many slots can are used.", alias="usedSlots"
    )
    moved_slots: Annotated[int, Field(strict=True, ge=0)] = Field(
        description="The slots that where moved to another activation code",
        alias="movedSlots",
    )
    original_slots: Annotated[int, Field(strict=True, ge=-1)] = Field(
        ...
        _obj = cls.model_validate(
            {
                ...
                "pipelineBundle": PipelineBundle.from_dict(obj["pipelineBundle"])
                if obj.get("pipelineBundle") is not None
                else None,
                "usages": [
                    ActivationCodeDetailUsage.from_dict(_item)
                    for _item in obj["usages"]
                ]
                if obj.get("usages") is not None
                else None,
            }
        )
        return _obj
