...
class ProjectPermissionV4(BaseModel):
    ...
    role_project: StrictStr = Field(
        description="Possible values are: NONE, VIEWER, CONTRIBUTOR, ADMINISTRATOR, DATA_PROVIDER. More types could be added in a future release.",
        alias="roleProject",
    )
    role_flow: StrictStr = Field(
        description="Possible values are: NONE, VIEWER, CONTRIBUTOR. More types could be added in a future release.",
        alias="roleFlow",
    )
    role_base: StrictStr = Field(
        description="Possible values are: NONE, VIEWER, CONTRIBUTOR. More types could be added in a future release.",
        alias="roleBase",
    )
    role_bench: StrictStr = Field(
        description="Possible values are: NONE, CONTRIBUTOR, ADMINISTRATOR. More types could be added in a future release.",
        alias="roleBench",
    )
    ...
    invitation_accepted: Optional[StrictBool] = Field(
        default=None,
        description="Only present when membershipType is EMAIL",
        alias="invitationAccepted",
    )
    invitation_rejected: Optional[StrictBool] = Field(
        default=None,
        description="Only present when user is invited by EMAIL",
        alias="invitationRejected",
    )
    ...
    def membership_type_validate_enum(cls, value):
        ...
        if value not in set(["USER", "EMAIL", "WORKGROUP"]):
            raise ValueError(
                "must be one of enum values ('USER', 'EMAIL', 'WORKGROUP')"
            )
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.invitation_accepted is None
            and "invitation_accepted" in self.model_fields_set
        ):
        ...
        # and model_fields_set contains the field
        if (
            self.invitation_rejected is None
            and "invitation_rejected" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "workgroup": Workgroup.from_dict(obj["workgroup"])
                if obj.get("workgroup") is not None
                else None,
