...
class CreateStorageCredential(BaseModel):
    ...
    name: Annotated[str, Field(min_length=1, strict=True, max_length=150)] = Field(
    ...
    aws_credentials: Optional[AwsCredentials] = Field(
        default=None, alias="awsCredentials"
    )
        ...
        _obj = cls.model_validate(
            {
                "name": obj.get("name"),
                "type": obj.get("type"),
                "awsCredentials": AwsCredentials.from_dict(obj["awsCredentials"])
                if obj.get("awsCredentials") is not None
                else None,
            }
        )
        return _obj
