...
class AnalysisInputExternalData(BaseModel):
    ...
    mount_path: Optional[StrictStr] = Field(
        default=None,
        description="The mount path is the location where the input file will be located on the machine that is running the pipeline. The use of a relative path is encouraged, but an absolute path is also allowed. The path should end with the file name, which may differ from the original input data.",
        alias="mountPath",
    )
    s3_details: Optional[AnalysisS3DataDetails] = Field(default=None, alias="s3Details")
    basespace_details: Optional[AnalysisBaseSpaceDataDetails] = Field(
        default=None, alias="basespaceDetails"
    )
    ...
    def type_validate_regular_expression(cls, value):
        ...
        if not re.match(r"s3|http|basespace", value):
            raise ValueError(
                r"must validate the regular expression /s3|http|basespace/"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "s3Details": AnalysisS3DataDetails.from_dict(obj["s3Details"])
                if obj.get("s3Details") is not None
                else None,
                "basespaceDetails": AnalysisBaseSpaceDataDetails.from_dict(
                    obj["basespaceDetails"]
                )
                if obj.get("basespaceDetails") is not None
                else None,
            }
        )
        return _obj
