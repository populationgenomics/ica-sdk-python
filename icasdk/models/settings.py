...
class Settings(BaseModel):
    ...
    actual_instance: Optional[
        Union[IntegerSettings, OptionSettings, StringSettings]
    ] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, StringSettings):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `StringSettings`"
            )
        else:
            match += 1
        ...
        if not isinstance(v, IntegerSettings):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `IntegerSettings`"
            )
        else:
            match += 1
        ...
        if not isinstance(v, OptionSettings):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `OptionSettings`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in Settings with oneOf schemas: IntegerSettings, OptionSettings, StringSettings. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in Settings with oneOf schemas: IntegerSettings, OptionSettings, StringSettings. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into Settings with oneOf schemas: IntegerSettings, OptionSettings, StringSettings. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Settings with oneOf schemas: IntegerSettings, OptionSettings, StringSettings. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        ...
        Union[Dict[str, Any], IntegerSettings, OptionSettings, StringSettings]
    ]:
