...
class AnalysisV4(BaseModel):
    ...
    reference: Annotated[str, Field(min_length=1, strict=True, max_length=255)] = Field(
        description="The unique reference of the analysis"
    )
    user_reference: Annotated[str, Field(min_length=1, strict=True, max_length=255)] = (
        Field(description="The user reference of the analysis", alias="userReference")
    )
    pipeline: PipelineV4
    workflow_session: Optional[WorkflowSessionV4] = Field(
        default=None, alias="workflowSession"
    )
    ...
    end_date: Optional[datetime] = Field(
        default=None, description="When the analysis was finished", alias="endDate"
    )
    summary: Optional[StrictStr] = Field(
        default=None, description="The summary of the analysis"
    )
    analysis_storage: Optional[AnalysisStorageV4] = Field(
        default=None, alias="analysisStorage"
    )
    analysis_priority: Optional[Annotated[str, Field(strict=True)]] = Field(
        default=None,
    ...
    def status_validate_regular_expression(cls, value):
        ...
        if not re.match(
            r"REQUESTED|QUEUED|INITIALIZING|PREPARING_INPUTS|IN_PROGRESS|GENERATING_OUTPUTS|AWAITING_INPUT|ABORTING|SUCCEEDED|FAILED|FAILED_FINAL|ABORTED",
            value,
        ):
            raise ValueError(
                r"must validate the regular expression /REQUESTED|QUEUED|INITIALIZING|PREPARING_INPUTS|IN_PROGRESS|GENERATING_OUTPUTS|AWAITING_INPUT|ABORTING|SUCCEEDED|FAILED|FAILED_FINAL|ABORTED/"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "owner": UserIdentifier.from_dict(obj["owner"])
                if obj.get("owner") is not None
                else None,
                "tenant": TenantIdentifier.from_dict(obj["tenant"])
                if obj.get("tenant") is not None
                else None,
                "reference": obj.get("reference"),
                "userReference": obj.get("userReference"),
                "pipeline": PipelineV4.from_dict(obj["pipeline"])
                if obj.get("pipeline") is not None
                else None,
                "workflowSession": WorkflowSessionV4.from_dict(obj["workflowSession"])
                if obj.get("workflowSession") is not None
                else None,
                "status": obj.get("status"),
                "startDate": obj.get("startDate"),
                "endDate": obj.get("endDate"),
                "summary": obj.get("summary"),
                "analysisStorage": AnalysisStorageV4.from_dict(obj["analysisStorage"])
                if obj.get("analysisStorage") is not None
                else None,
                "analysisPriority": obj.get("analysisPriority"),
                "tags": AnalysisTag.from_dict(obj["tags"])
