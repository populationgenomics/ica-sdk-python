...
class DataTransfer(BaseModel):
    ...
    data_transferred: Annotated[int, Field(strict=True, ge=0)] = Field(
        description="The data transferred so far in bytes.", alias="dataTransferred"
    )
    status: StrictStr
    status_message: Optional[
        ...
        default=None,
        description="A message explaining the reason why the transfer is in the current status.",
        alias="statusMessage",
    )
    ...
    def direction_validate_enum(cls, value):
        ...
        if value not in set(["UPLOAD", "DOWNLOAD", "IMPORT"]):
            raise ValueError(
                "must be one of enum values ('UPLOAD', 'DOWNLOAD', 'IMPORT')"
            )
        return value
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(
            ...
            raise ValueError(
                "must be one of enum values ('REQUESTED', 'ONGOING', 'SUCCEEDED', 'FAILED', 'ABORTED', 'ABORTREQUESTED', 'SCHEDULED')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "connector": Connector.from_dict(obj["connector"])
