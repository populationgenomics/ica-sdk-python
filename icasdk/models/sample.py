...
class Sample(BaseModel):
    ...
    ] = Field(default=None, description="The description of the sample")
    tags: SampleTag
    region: Region
    application: Optional[ApplicationV4] = None
    status: StrictStr
    metadata_valid: StrictBool = Field(
        description="Whether the metadata is valid", alias="metadataValid"
    )
    metadata: List[MetadataField] = Field(description="The metadata of the sample")
    sequencing_runs: Optional[List[Optional[SequencingRun]]] = Field(
        default=None, alias="sequencingRuns"
    )
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(["DELETED", "AVAILABLE", "PARTIAL"]):
            raise ValueError(
                "must be one of enum values ('DELETED', 'AVAILABLE', 'PARTIAL')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "tags": SampleTag.from_dict(obj["tags"])
                ...
                "metadata": [
                    MetadataField.from_dict(_item) for _item in obj["metadata"]
                ]
                if obj.get("metadata") is not None
                else None,
                "sequencingRuns": [
                    SequencingRun.from_dict(_item) for _item in obj["sequencingRuns"]
                ]
                if obj.get("sequencingRuns") is not None
                else None,
            }
        )
        return _obj
