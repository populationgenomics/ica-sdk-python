...
class FolderUploadSession(BaseModel):
    ...
    time_session_expires: datetime = Field(
        description="The time the folder upload session will expire as it is only temporarily valid.",
        alias="timeSessionExpires",
    )
    ...
    time_closed: Optional[datetime] = Field(
        default=None,
        description="The time the folder upload session was closed.",
        alias="timeClosed",
    )
    temp_credentials: Optional[TempCredentials] = Field(
        default=None, alias="tempCredentials"
    )
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(["OPEN", "CLOSED", "COMPLETED"]):
            raise ValueError(
                "must be one of enum values ('OPEN', 'CLOSED', 'COMPLETED')"
            )
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.temp_credentials is None
            and "temp_credentials" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "tempCredentials": TempCredentials.from_dict(obj["tempCredentials"])
                if obj.get("tempCredentials") is not None
                else None,
            }
        )
        return _obj
